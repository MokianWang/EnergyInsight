...
def research_all(sub_questions: list[dict], question_type: str) -> dict:
    """
    对所有子问题执行研究（按依赖关系排序）
    ...
    try:
        for sq in ordered:
            if use_mcp:
                result = research_single_mcp(sq, question_type, mcp_tools)
            else:
                result = research_single_fallback(sq, question_type)
